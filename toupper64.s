...
open_fd_in:
 ...
 movq  $0666, %rdx             #set permissions. this doesn't really matter for reading
 syscall
...
open_fd_out:
 ...
 movq  $O_CREAT_WRONLY_TRUNC, %rsi   #write flags
...
read_loop_begin:
 ...
 movq  $SYS_READ, %rax          #get the input file descriptor
 movq  ST_FD_IN(%rbp), %rdi     #file descriptor
 movq  $BUFFER_DATA, %rsi       #buffer location
 movq  $BUFFER_SIZE, %rdx       #buffer size
 syscall                        #size of buffer read is returned in %rax

 ###EXIT IF WE'VE REACHED THE END###
 #check for end of file marker
 cmpq $END_OF_FILE, %rax        #if found or on error, go to the end
 jle   end_loop
...
continue_read_loop:
 ...
 movq  %rax, %rdx              #size of the buffer to write
 movq  $SYS_WRITE, %rax        #prime syscall
